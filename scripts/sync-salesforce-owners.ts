...
import type { SalesforceOwner } from "@/app/lib/owners"
...
async function main() {
  ...
  await writeFile(
    outputPath,
    `${JSON.stringify(mergedOwners, null, 2)}\n`,
    "utf8",
  )
  console.log(
    `Added ${newOwners.length} new Salesforce owners; ${mergedOwners.length} total saved`,
  )
}
